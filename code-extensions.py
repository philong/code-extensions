...
from typing import IO, NamedTuple, NotRequired, TextIO, TypedDict
...
try:
    ...
    sys.exit(
        ...
        f"tomllib not available on current Python {platform.python_version()}."
    )
...
class CliOption(NamedTuple):
    ...
    # add_argument's keyword options passed through to parser.add_argument.
...
def _switch(
    ...
    kwargs: AddArgumentKwargs = {"action": "store_true", "default": default}
    if dest is not None:
        kwargs["dest"] = dest
    return CliOption(flags, help_text, summary, None, kwargs)
...
def _valued(
    ...
    kwargs: AddArgumentKwargs = {"default": default}
    if type is not None:
        kwargs["type"] = type
    if dest is not None:
        kwargs["dest"] = dest
    return CliOption(flags, help_text, summary, value, kwargs)
